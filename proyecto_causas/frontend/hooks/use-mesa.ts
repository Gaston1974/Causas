...
export function useMesa() {
    ...
    const [entradas, setEntradas] = useState<MesaEntradaResponse[]>([])
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)
    const [search, setSearch] = useState("")

    const fetchEntradas = useCallback(async (): Promise<MesaEntradasListResponse> => {
        try {
            setLoading(true);
            setError(null);
            ...
            setError("Error al obtener entradas");
            throw err;
        } finally {
            setLoading(false);
        }
